...
use crate::core::{
    BaseDataGetters, Event, EventData, EventType, ExchangeType, Instrument, Manager, OrderFlag,
    OrderManager, OrderType, Side, Trade,
};
...
impl<'a> OrderBook<'a> for LimitOrderBook<'a> {
    ...
    fn modify(&mut self, order_id: Id, volume: Float) -> VecDeque<Event> {
        ...
        let mut events = VecDeque::new();
        let existing_order = self.order_manager.get(order_id);
        ...
        self.order_manager.replace(order_id, replacement_order);
    ...
    fn cancel(&mut self, order_id: Id) -> VecDeque<Event> {
        let mut events = VecDeque::new();
        let order = self.order_manager.get(order_id);
...
impl LimitOrderBook<'_> {
    fn volume_at_level(&self, level: Float) -> Float {
        self.levels
            ...
            .unwrap()
            .iter()
            .map(|&order_id| self.order_manager.get(order_id).get_volume_left())
            .sum()
    }
    ...
    fn add_internal(&mut self, taker_order_id: Id) -> VecDeque<Event> {
        ...
        let order = self.order_manager.get(taker_order_id);
        ...
        loop {
            ...
            if crossing {
                ...
                let maker_order = self.order_manager.get(maker_order_id);
        ...
        if undo {
            ...
            if filled_so_far_in_txn > Float::from_int(0) {
                ...
                filled.iter().for_each(|&(order_id, to_add_to_fill)| {
                    ...
                    self.order_manager.set_filled(order_id, to_add_to_fill)
                });
                partial
                    .iter()
